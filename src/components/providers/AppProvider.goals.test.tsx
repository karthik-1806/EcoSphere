...
function GoalsConsumer() {
  ...
  const addGoal = () => dispatch.addGoal({ id: "g1", title: "G1", category: "total", targetValue: 100, progress: 0, active: true, createdAt: new Date().toISOString() });
  const toggle = () => dispatch.toggleGoalActive("g1");
  const addChallenge = () => dispatch.addChallenge({ id: "c1", title: "C1", description: "d", durationDays: 7, completed: false, createdAt: new Date().toISOString() });
  ...
  return (
    <div>
      ...
      <button onClick={addGoal} data-testid="add-goal">add-goal</button>
      <button onClick={toggle} data-testid="toggle-goal">toggle-goal</button>
      <button onClick={addChallenge} data-testid="add-challenge">add-challenge</button>
      <button onClick={complete} data-testid="complete-challenge">complete-challenge</button>
    </div>
  );
}
